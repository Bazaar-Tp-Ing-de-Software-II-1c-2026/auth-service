...
def setup_pin(user_id: int, payload: schemas.PINSetupRequest, db: Session) -> dict:
    ...
    """
    logger.debug(f"[PIN SERVICE] Setting up PIN for user_id={user_id}, device_id={payload.device_id}")
...
def login_with_pin(payload: schemas.PINLoginRequest, db: Session) -> dict:
    ...
    user = db.query(models.User).filter(
        models.User.pin_device_id == payload.device_id
    ).first()
    ...
    if not security.verify_password(payload.pin, user.pin_hash):
        ...
        if user.pin_failed_attempts >= PIN_MAX_FAILED_ATTEMPTS:
            ...
            logger.warning(f"[PIN SERVICE] PIN locked for user_id={user.id} after {PIN_MAX_FAILED_ATTEMPTS} failed attempts")
            raise ServiceException(
                status_code=403,
                title="Forbidden",
        ...
        logger.warning(f"[PIN SERVICE] Failed PIN attempt for user_id={user.id}, remaining attempts: {remaining_attempts}")
        raise ServiceException(
            status_code=401,
            title="Unauthorized",
...
def remove_pin(user_id: int, payload: schemas.PINRemoveRequest, db: Session) -> dict:
    ...
    """
    logger.debug(f"[PIN SERVICE] Removing PIN for user_id={user_id}, device_id={payload.device_id}")
